...
- Text type: `str`
      - Methods: *upper()*, *lower()*, *strip()*, *replace()*, *split()*, *join()*, *find()*, capitalize(), casefold(), center(), count(), encode(), endswith(), expandtabs(), find(), format(), format_map(), index(), isalnum(), isalpha(), isascii(), isdecimal(), isdigit(), isidentifier(), islower(), isnumeric(), isprintable(), istitle(), isupper(), ljust(), lstrip(), maketrans(), partition(), rfind(), rindex(), rjust(), rpartition(), rsplit(), rstrip(), splitlines(), startswith(), swapcase(), title(), translate(), zfill()
